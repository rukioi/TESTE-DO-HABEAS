...
const projectSchema = z.object({
  ...
  clientName: z.string({ required_error: 'Cliente é obrigatório' }).min(1, 'Cliente é obrigatório'),
  ...
  status: z.enum([ 'contacted', 'proposal', 'won', 'lost' ], { required_error: 'Status é obrigatório' }),
...
interface ProjectFormProps {
  ...
  onSubmit: (data: ProjectFormData & { tags: string[]; contacts: ProjectContact[]; attachments?: File[]; removeAttachments?: string[] }) => void;
  isEditing?: boolean;
...
export function ProjectForm({ open, onOpenChange, project, onSubmit, isEditing = false, existingTags = [] }: ProjectFormProps) {
  ...
  const form = useForm<ProjectFormData>({
    ...
    defaultValues: {
      ...
      priority: String(project?.priority) === 'urgent' ? 'high' : (project?.priority || 'medium'),
  ...
  const handleSubmit = (data: ProjectFormData) => {
    onSubmit({ ...data, tags, contacts, attachments: projectFiles, removeAttachments: attachmentsToRemove });
    onOpenChange(false);
    form.reset();
    setTags([]);
    setContacts([]);
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      if (!open || !isEditing || !project?.id) return;
      try {
